...
{
    public class CallbackShippingOptionUpdateTest
    {
        [Fact]
        public void CanReadRequestJson()
        {
            var json = File.ReadAllText(Path.Combine(System.Environment.CurrentDirectory, "Data", "CallbackShippingOptionUpdateRequest.json"));
        ...
        {
            var json = File.ReadAllText(Path.Combine(System.Environment.CurrentDirectory, "Data", "CallbackShippingOptionUpdateResponse.json"));
            Assert.True(!string.IsNullOrEmpty(json));
        }
